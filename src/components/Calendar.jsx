...
function Calendar() {
  ...
  const createCreneau = async (date, heure) => {
    ...
    const { data, error } = await supabase
      .from('creneaux')
      .insert([{ date: dateFormatee, heure: heureFormatee, statut: 'disponible' }])
      .select()
              ...
              <tr key={heure}>
                ...
                {joursSemaine.map((date, i) => {
                  ...
                  const bgColor = creneau
                    ? (creneau.statut === 'disponible' ? 'bg-green-400' : 'bg-red-400')
                    : 'hover:bg-gray-100'
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-white p-6 rounded shadow-lg">
            ...
            <div className="flex justify-end gap-4">
              ...
              <button
                onClick={() => createCreneau(selectedSlot.date, selectedSlot.heure)}
